...
fontScoreHist = pygame.font.SysFont("comicsansms", 18)
fontDebug = pygame.font.SysFont("comicsansms", 10)
...
def addMark(p1, t, remove=False):
    ...
    if tMark == 2:
        ...
        pygame.draw.circle(screen, (clr, clr, clr), (xTarget, yTarget), round(min(colMark, rowHeight)/2), 3)
...
def scoreMultiplierTarget(p1, sm):
    ...
    while not done:
        for event in pygame.event.get():
            # JOYSTICK INPUTS
            if event.type == pygame.JOYAXISMOTION and joystick.get_axis(1) == -1: # DOWN #todo include this logic in "increaseplayertotal"
            ...
            if event.type == pygame.JOYAXISMOTION and joystick.get_axis(0) == -1: # LEFT
                otherTarget += 1
            if event.type == pygame.JOYAXISMOTION and joystick.get_axis(0) == 1: # RIGHT
                otherTarget += 4
            if event.type == pygame.JOYAXISMOTION and joystick.get_axis(1) == 1: # UP
                otherTarget *= 2
...
def scoreCorrection(p1):
    ...
    while not done:
        for event in pygame.event.get():
            ...
            # JOYSTICK INPUTS
            if event.type == pygame.JOYAXISMOTION and joystick.get_axis(0) == -1: # DOWN
                updateRound(currentRound)
                if p1:
            ...
            if event.type == pygame.JOYAXISMOTION and joystick.get_axis(1) == -1: # LEFT
                otherTarget -= 1
            if event.type == pygame.JOYAXISMOTION and joystick.get_axis(1) == 1: # RIGHT
                otherTarget -= 4
            if event.type == pygame.JOYAXISMOTION and joystick.get_axis(0) == 1: # UP
                otherTarget *= 2
            if event.type == pygame.JOYBUTTONDOWN and joystick.get_button(0):
                addMark(p1,"20",True)
    ...
    if n < 41:
        if p1:
           ...
           # x = round((m.floor(n/21)+1) * colScore/4 - mark.get_width()/2)
            y = rowTop + round(((n-1) % 20) * rowHeight /2)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_DOWN:
            ...
            if player1:
                ...
                updateRound(currentRound)
        # JOYSTICK INPUTS
        if event.type == pygame.JOYAXISMOTION and joystick.get_axis(1) == -1: # DOWN
            player1 = not player1
            changePlayer(player1)
            scoreMultiplier = 1
            if player1:
